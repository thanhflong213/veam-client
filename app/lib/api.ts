...
export function adminGetAnnouncements(
  ...
  return apiFetch<AnnouncementsResponse>("/announcements/manage?limit=100", {
    headers: authHeaders(token),
  });
}

export function adminGetAnnouncement(
  id: string,
  token: string,
): Promise<Announcement> {
  return apiFetch<Announcement>(`/announcements/manage/${id}`, {
    headers: authHeaders(token),
  });
}
...
export function adminUpdateSettings(
  ...
  const { _id, createdAt, updatedAt, __v, ...payload } = data as Record<
    string,
    unknown
  >;
  void _id;
...
export function getInstitutions(
  ...
  if (params?.limit) q.set("limit", String(params.limit));
  if (params?.search) q.set("search", params.search);
  return apiFetch<InstitutionsResponse>(
    `/institutions${q.toString() ? "?" + q : ""}`,
  );
}
...
export function adminGetInstitutions(
  token: string,
): Promise<InstitutionsResponse> {
  return apiFetch<InstitutionsResponse>("/institutions/manage?limit=100", {
    headers: authHeaders(token),
  });
}

export function adminGetInstitution(
  id: string,
  token: string,
): Promise<Institution> {
  return apiFetch<Institution>(`/institutions/manage/${id}`, {
    headers: authHeaders(token),
  });
}

export function adminCreateInstitution(
...
export function adminUpdateInstitution(
  id: string,
  data: Partial<Institution>,
  token: string,
): Promise<Institution> {
...
export function adminDeleteInstitution(
  id: string,
  token: string,
): Promise<void> {
  return apiFetch<void>(`/institutions/${id}`, {
    method: "DELETE",
    headers: authHeaders(token),
  });
}
