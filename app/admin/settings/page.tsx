...
import {
  faFloppyDisk, faPlus, faChevronUp, faChevronDown, faTrash, faChevronRight,
...
function SectionHeader({ title, onAdd, addLabel, isOpen, onToggle }: { title: string; onAdd?: () => void; addLabel?: string; isOpen?: boolean; onToggle?: () => void }) {
  return (
    <div className="a-hdr" style={{ marginTop: 8, cursor: onToggle ? 'pointer' : 'default' }} onClick={onToggle}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 18, color: 'var(--navy)', margin: 0 }}>{title}</h2>
      </div>
      {onAdd && (
        <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); onAdd(); }} className="gap-1.5">
...
export default function AdminSettingsPage() {
  ...
  const [collapsed, setCollapsed] = useState<Record<string, boolean>>({
    'site-theme': false,
    'conference-info': false,
    'contact-info': false,
    'social-links': false,
    'hero-slides': false,
    'important-dates': false,
    'keynote-speakers': false,
    'special-sessions': false,
    'publications': false,
    'organizing-institutions': false,
    'navigation-menu': false,
    'paper-archives': false,
  });
  ...
  return (
    <>
      <div className="a-hdr" style={{ position: 'sticky', top: 0, zIndex: 100, background: 'white', borderBottom: '1px solid var(--border)', paddingBottom: 16 }}>
        <h2>Settings</h2>
      ...
      <div className="settings-section">
        <SectionHeader title="Site and Theme" isOpen={!collapsed['site-theme']} onToggle={() => toggleSection('site-theme')} />
        {!collapsed['site-theme'] && (
      ...
      {/* ── Conference Info ── */}
      <div className="settings-section">
        <SectionHeader title="Conference Info" isOpen={!collapsed['conference-info']} onToggle={() => toggleSection('conference-info')} />
        {!collapsed['conference-info'] && (
          <Card><CardContent className="pt-5">
      ...
      {/* ── Contact Info ── */}
      <div className="settings-section">
        <SectionHeader title="Contact Info" isOpen={!collapsed['contact-info']} onToggle={() => toggleSection('contact-info')} />
      ...
      {/* ── Social Links ── */}
      <div className="settings-section">
        <SectionHeader title="Social Links" isOpen={!collapsed['social-links']} onToggle={() => toggleSection('social-links')} />
      ...
      {/* ── Hero Slides ── */}
      <div className="settings-section">
        <SectionHeader title="Hero Slides" onAdd={addSlide} addLabel="Add Slide" isOpen={!collapsed['hero-slides']} onToggle={() => toggleSection('hero-slides')} />
        {!collapsed['hero-slides'] && (
          <>
      ...
      {/* ── Important Dates ── */}
      <div className="settings-section">
        <SectionHeader title="Important Dates" onAdd={() => addToArr<ImportantDate>('importantDates', { date: '', title: '', description: '' })} addLabel="Add Date" isOpen={!collapsed['important-dates']} onToggle={() => toggleSection('important-dates')} />
        {!collapsed['important-dates'] && (
          <>
            {dates.map((item, idx) => (
              <div key={idx} className="slide-editor-item">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
      ...
      {/* ── Keynote Speakers ── */}
      <div className="settings-section">
        <SectionHeader title="Keynote Speakers" onAdd={() => addToArr<Keynote>('keynotes', { name: '', institution: '', topic: '' })} addLabel="Add Speaker" isOpen={!collapsed['keynote-speakers']} onToggle={() => toggleSection('keynote-speakers')} />
        {!collapsed['keynote-speakers'] && (
          <>
            {keynotes.map((sp, idx) => (
              <div key={idx} className="slide-editor-item">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
      ...
      {/* ── Special Sessions ── */}
      <div className="settings-section">
        <SectionHeader title="Special Sessions" onAdd={() => addToArr<SpecialSession>('specialSessions', { title: '', chair: '' })} addLabel="Add Session" isOpen={!collapsed['special-sessions']} onToggle={() => toggleSection('special-sessions')} />
        {!collapsed['special-sessions'] && (
          <>
            {sessions.map((sess, idx) => (
              <div key={idx} className="slide-editor-item">
                ...
                <div className="form-row" style={{ marginBottom: 0 }}>
                  <div className="form-group" style={{ margin: 0 }}>
                    <Label>Title</Label>
                    <Input type="text" value={sess.title} onChange={e => updateArr<SpecialSession>('specialSessions', idx, { title: e.target.value })} placeholder="Session title" />
                  </div>
                  <div className="form-group" style={{ margin: 0 }}>
                    <Label>Chair</Label>
                    <Input type="text" value={sess.chair} onChange={e => updateArr<SpecialSession>('specialSessions', idx, { chair: e.target.value })} placeholder="Chair: Name, University, Country" />
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* ── Publications ── */}
      <div className="settings-section">
        <SectionHeader title="Publications" onAdd={() => addToArr<Publication>('publications', { title: '', description: '' })} addLabel="Add Publication" isOpen={!collapsed['publications']} onToggle={() => toggleSection('publications')} />
              ...
              <div key={idx} className="slide-editor-item">
                ...
                <div className="form-row" style={{ marginBottom: 0 }}>
                  <div className="form-group" style={{ margin: 0 }}>
                    ...
                    <Input type="text" value={pub.title} onChange={e => updateArr<Publication>('publications', idx, { title: e.target.value })} placeholder="Journal name" />
                  </div>
                  <div className="form-group" style={{ margin: 0 }}>
                    <Label>Description</Label>
                    <Input type="text" value={pub.description ?? ''} onChange={e => updateArr<Publication>('publications', idx, { description: e.target.value })} placeholder="Short description" />
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="settings-section">
        <SectionHeader title="Organizing Institutions" onAdd={() => addToArr<OrganizingInstitution>('organizingInstitutions', { name: '', role: '' })} addLabel="Add Institution" isOpen={!collapsed['organizing-institutions']} onToggle={() => toggleSection('organizing-institutions')} />
        {!collapsed['organizing-institutions'] && (
          <>
            {orgs.map((org, idx) => (
              <div key={idx} className="slide-editor-item">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
      ...
      {/* ── Navigation Menu ── */}
      <div className="settings-section">
        <SectionHeader title="Navigation Menu" isOpen={!collapsed['navigation-menu']} onToggle={() => toggleSection('navigation-menu')} />
      ...
      {/* ── Paper Archives Sidebar ── */}
      <div className="settings-section">
        <SectionHeader title="Paper Archives Sidebar" isOpen={!collapsed['paper-archives']} onToggle={() => toggleSection('paper-archives')} />
        {!collapsed['paper-archives'] && (
          <Card><CardContent className="pt-5">
            <p style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, color: 'var(--text-muted)', marginBottom: 12 }}>
